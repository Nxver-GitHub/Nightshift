...
1. **The VM never holds a real credential.** Keys are bound through Superserve *Secrets*: the
   ...
   the provider. Even a fully compromised company VM cannot leak its own Anthropic or Stripe key.
...
| `anthropic-key` *(Superserve secret name)* | Superserve console → Secrets | bound to `ANTHROPIC_API_KEY` in the VM |
